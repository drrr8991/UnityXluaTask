...
task.print = function (...)
    ...
    local t = {}
    t[#t+1] = os.date("[%Y-%m-%d %H:%M:%S]")..string.format("[%08x]", core.id)
    for _, v in ipairs({...}) do
        t[#t+1] = tostring(v)
    end
    t[#t+1] =string.format("(%s:%d)", info.source, info.currentline)
    core.log(table.concat(t,'\t'))
end
...
if core.id == MAIN_ID then -- main task
    ...
    ---@return integer
    function task.new(name, sourceFile)
        return manager_core.new(name, sourceFile)
    end
...
---@param PTYPE string
---@param fn fun(sender:integer, data:string, session:integer)
---@return boolean
